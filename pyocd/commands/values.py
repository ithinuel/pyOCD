...
class TargetValue(ValueBase):
    ...
    def display(self, args):
        self.context.write(
            f"Target type:  {self.context.session.options.get('target_override')}"
        )
                    ...
                    [
                        (
                            (
                                "*"
                                if (
                                    (self.context.selected_core is not None)
...
class MemoryMapValue(ValueBase):
    ...
    def display(self, args):
        ...
        def add_rows(
            indent: int, pt: prettytable.PrettyTable, map: "MemoryMap"
        ) -> None:
            for region in map:
                pt.add_row(
                    [
...
class PeripheralsValue(ValueBase):
    ...
    def display(self, args):
        for periph in sorted(
            self.context.peripherals.values(), key=lambda x: x.base_address
        ):
...
class AccessiblePinsValue(ValueBase):
    ...
    def display(self, args):
        ...
        self.context.write(
            f"  SWCLK/TCK = {pin_desc(DebugProbe.ProtocolPin.SWCLK_TCK)}"
        )
        self.context.write(
            f"  SWDIO/TMS = {pin_desc(DebugProbe.ProtocolPin.SWDIO_TMS)}"
        )
...
class PinsValue(ValueBase):
    INFO = {
        ...
        "extra_help": "The pins value is a mask containing the state of all accessible protocol pins. "
    ...
    def print_current_pin_values(self):
        pins = self.context.probe.read_pins(
            DebugProbe.PinGroup.PROTOCOL_PINS, DebugProbe.ProtocolPin.ALL_PINS
        )
    ...
    def modify(self, args):
        ...
        state = int(args[0], base=0)
        self.context.probe.write_pins(
            DebugProbe.PinGroup.PROTOCOL_PINS, DebugProbe.ProtocolPin.ALL_PINS, state
        )
        self.print_current_pin_values()
...
class MemApValue(ValueBase):
    ...
    def display(self, args):
        if self.context.selected_ap is None:
            ...
            return
        self.context.writef(
            "{} is selected", self.context.selected_ap.short_description
        )
...
class HnonsecValue(ValueBase):
    INFO = {
        ...
        "extra_help": "This value controls whether memory transactions are secure or nonsecure. The value is an "
...
class HprotValue(ValueBase):
    INFO = {
        ...
        "extra_help": """This integer value controls attributes of memory transfers. It is a direct mapping of the AHB
...
Manual for your device's MEM-AP for details.""",
    ...
    def display(self, args):
        ...
        self.context.writef(
            "{} HPROT = {:#x}", self.context.selected_ap.short_description, hprot
        )
        desc = ""
        for bitnum in range(7):
            bitvalue = (hprot >> bitnum) & 1
            desc += "    HPROT[{}] = {:#x} ({})\n".format(
                bitnum, bitvalue, HPROT_BIT_DESC[bitnum][bitvalue]
            )
...
class StepIntoInterruptsValue(ValueBase):
    ...
    def display(self, args):
        self.context.writei(
            "Interrupts while stepping: %s",
            (
                "enabled"
                if self.context.session.options["step_into_interrupt"]
                else "disabled"
            ),
        )
    ...
    def modify(self, args):
        ...
        if level_name not in self.LEVELS:
            raise exceptions.CommandError(
                "log level must be one of {%s}" % ",".join(self.LEVELS.keys())
            )
        ...
        if len(args) == 1:
            ...
            for module_name in args[1:]:
                ...
                if hasattr(logging.root, "manager"):
                    ...
                    if hasattr(manager, "loggerDict") and (
                        module_name not in manager.loggerDict
                    ):
                        self.context.writef(
                            "No logger exists with the name {}", module_name
                        )
                        continue
...
class DebugSequencesValue(ValueBase):
    ...
    def display(self, args):
        ...
        for seq in sorted(
            self.context.target.debug_sequence_delegate.all_sequences,
...
class ResetTypeValue(ValueBase):
    ...
    def display(self, args):
        ...
        self.context.write(
            f"Selected reset type ('reset_type' option): {reset_type_desc}"
        )
        ...
        for core in self.context.target.cores.values():
            ...
            self.context.write(
                f"  Default SW: {cm_core.default_software_reset_type.name}"
            )
            self.context.write(
                "  Available:  "
                + ", ".join(r.name for r in cm_core.supported_reset_types)
            )
