...
class DebugProbeServer(threading.Thread):
    ...
    def run(self) -> None:
        ...
        LOG.info(
            "Serving debug probe %s (%s) on port %i",
            self._probe.description,
            self._probe.unique_id,
            self._port,
        )
...
class TCPProbeServer(ThreadingTCPServer):
    ...
    def __init__(
        self, server_address: Tuple[str, int], session: "Session", probe: DebugProbe
    ):
        self._session = session
        self._probe = probe
        super().__init__(
            server_address, DebugProbeRequestHandler, bind_and_activate=False
        )
    ...
    def handle_error(self, request, client_address):
        LOG.error(
            "Error while handling client request (client address %s):",
            client_address,
...
class DebugProbeRequestHandler(StreamRequestHandler):
    ...
    def setup(self):
        ...
        LOG.info(
            "Client %s (port %i) connected to probe %s",
            self._client_domain,
    ...
    def finish(self):
        LOG.info(
            "Client %s (port %i) disconnected from probe %s",
            self._client_domain,
            self.client_address[1],
            self._probe.unique_id,
        )
        ...
        while True:
            ...
            try:
                ...
                if not isinstance(request_args, list):
                    self._send_error_response(
                        message="invalid request arguments format"
                    )
                    continue
            ...
            # Catch all exceptions so that an error response can be returned, to not leave the client hanging.
                ...
                if request is not None:
                    LOG.error(
                        "Error processing '%s' request (ID %i, client %s, probe %s): %s",
                        request_type,
                    ...
                    LOG.debug(
                        "Full request from error: %s",
                        request.decode("utf-8", "replace"),
                    )
                    self._send_error_response(
                        status=self._get_exception_status_code(err), message=str(err)
                    )
                else:
                    LOG.error(
                        "Error before request was received: %s",
                        err,
                        exc_info=self._session.log_tracebacks,
                    )
    ...
    def _request__hello(self, version):
        ...
        if version != self.PROTOCOL_VERSION:
            raise exceptions.Error(
                "client requested unsupported protocol version %i (expected %i)"
    ...
    def _request__get_memory_interface_for_ap(
        self, ap_address_version, ap_nominal_address
    ):
        ...
        if ap_version == APVersion.APv1:
            ...
            raise exceptions.Error(
                "invalid AP version in remote get_memory_interface_for_ap request"
            )
