...
class ConnectHelper:
    ...
    def get_sessions_for_all_connected_probes(
        ...
        **kwargs,
        ...
        probes = ConnectHelper.get_all_connected_probes(
            blocking=blocking, unique_id=unique_id
        )
    ...
    def get_all_connected_probes(
        ...
        while True:
            allProbes = DebugProbeAggregator.get_all_connected_probes(
                unique_id=unique_id
            )
            sortedProbes = sorted(
                allProbes, key=lambda probe: probe.description + probe.unique_id
            )
            ...
            if not blocking:
                ...
                if print_wait_message and not printedMessage:
                    if unique_id is None:
                        ...
                        msg = (
                            "Waiting for a debug probe matching unique ID '%s' to be connected..."
                            % unique_id
                        )
    ...
    def choose_probe(
        ...
        if (unique_id is not None) and (len(allProbes) > 1) and not return_first:
            print(
                colorama.Fore.RED
                + "More than one debug probe matches unique ID '%s':" % unique_id
                + colorama.Style.RESET_ALL
            )
            ...
            for probe in allProbes:
                ...
                highlightedId = (
                    head + colorama.Fore.RED + sep + colorama.Style.RESET_ALL + tail
                )
                print("%s | %s" % (probe.description, highlightedId))
            return None
        ...
        if allProbes is None or len(allProbes) == 0:
            if unique_id is None:
                ...
                print(
                    colorama.Fore.RED
                    + "No connected debug probe matches unique ID '%s'" % unique_id
                    + colorama.Style.RESET_ALL
                )
    ...
    def _print_probe_list(probes: Sequence["DebugProbe"]) -> None:
        ...
        dim_dash = (
            colorama.Style.DIM + colorama.Fore.WHITE + "n/a" + colorama.Style.RESET_ALL
        )
        ...
        for index, probe in enumerate(probes):
            try:
                ...
                if target_type_name is not None:
                    ...
                    has_target = (
                        target_type_name in TARGET
                    ) or is_pack_target_available(
                        target_type_name, Session.get_current()
                    )
                    target_mark = ("✖︎", "✔︎")[has_target]
                    target_color = (
                        colorama.Fore.LIGHTGREEN_EX if has_target else colorama.Fore.RED
                    )
                    ...
                    pt.add_row(
                        [
                            ...
                            (
                                ...
                                + (board_info.vendor or board_info.name)
                            )
                            if board_info
                            else dim_dash,
                            (colorama.Fore.YELLOW + board_info.name)
                            if (board_info and board_info.vendor)
                            else "",
                            "",
                        ]
                    )
