...
class Target(MemoryInterface, DelegateHavingMixIn, ABC):
    class State(Enum):
    ...
    class VectorCatch:
        ...
        STATE_ERR = 1 << 4
        # Trap on UsageFault exception caused by checking error, for example an alignment check
        # error.
        CHECK_ERR = 1 << 5
    ...
    def set_breakpoint(
        self, addr: int, type: BreakpointType = BreakpointType.AUTO
    ) -> bool:
        raise NotImplementedError()
    ...
    def remove_watchpoint(
        self, addr: int, size: Optional[int], type: Optional[WatchpointType]
    ) -> None:
        raise NotImplementedError()
