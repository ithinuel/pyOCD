...
class CoreSightTarget(SoCTarget):
    ...
    def primary_core_pname(self) -> str:
        ...
        for i, proc_info in enumerate(
            sorted(ap_map.values(), key=lambda p: p.ap_address)
        ):
            if i == primary_core:
                return proc_info.name
        else:
            raise exceptions.Error(
                f"invalid 'primary_core' session option '{primary_core}' "
    ...
    def irq_table(self) -> Dict[int, str]:
        if self._irq_table is None:
            if (self.svd_device is not None) and (
                self.svd_device.peripherals is not None
            ):
                peripherals = [
                    p for p in self.svd_device.peripherals if p.interrupts is not None
                ]
                self._irq_table = {
                    i.value: i.name for p in peripherals for i in p.interrupts
                }
            else:
                self._irq_table = {}
    ...
    def trace_start(self):
        ...
        if not result and self.has_debug_sequence(
            "TraceStart", pname=self.selected_core_or_raise.node_name
        ):
            assert self.debug_sequence_delegate
            self.debug_sequence_delegate.run_sequence(
                "TraceStart", pname=self.selected_core_or_raise.node_name
    ...
    def trace_stop(self):
        ...
        if not result and self.has_debug_sequence(
            "TraceStop", pname=self.selected_core_or_raise.node_name
        ):
            assert self.debug_sequence_delegate
            self.debug_sequence_delegate.run_sequence(
                "TraceStop", pname=self.selected_core_or_raise.node_name
