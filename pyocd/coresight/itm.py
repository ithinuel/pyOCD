...
class ITM(CoreSightComponent):
    ...
    def init(self):
        ...
        if (val & (ITM.LSR_SLK_MASK | ITM.LSR_SLI_MASK)) == (
            ITM.LSR_SLK_MASK | ITM.LSR_SLI_MASK
        ):
    ...
    def enable(self, enabled_ports=0xFFFFFFFF):
        self.ap.write32(
            self.address + ITM.TCR,
            (
                (1 << ITM.TCR_TRACEBUSID_SHIFT)
                | ITM.TCR_ITMENA_MASK
                | ITM.TCR_TSENA_MASK
                | ITM.TCR_TXENA_MASK
        ...
        self.ap.write32(
            self.address + ITM.TPR, 0
        )  # Allow unprivileged access to all 32 ports.
