...
class FreeRTOSThreadProvider(ThreadProvider):
    ...
    def init(self, symbolProvider):
        ...
        suspendedTaskListSym = self._lookup_symbols(
            ["xSuspendedTaskList"], symbolProvider
        )
        ...
        tasksWaitingTerminationSym = self._lookup_symbols(
            ["xTasksWaitingTermination"], symbolProvider
        )
        ...
        if delta != LIST_SIZE:
            LOG.warning(
                "FreeRTOS: list size is unexpected, maybe an unsupported configuration of FreeRTOS."
                + elfOptHelp
            )
            return False
        ...
        if delta % LIST_SIZE:
            LOG.warning(
                "FreeRTOS: pxReadyTasksLists size is unexpected, maybe an unsupported version of FreeRTOS."
                + elfOptHelp
            )
            return False
        ...
        if self._total_priorities > FREERTOS_MAX_PRIORITIES:
            LOG.warning(
                "FreeRTOS: number of priorities is too large (%d)." + elfOptHelp,
    ...
    def _build_thread_list(self):
        ...
        threadCount = self._target_context.read32(
        ...
        if "xSuspendedTaskList" in self._symbols:
            listsToRead.append(
                (self._symbols["xSuspendedTaskList"], FreeRTOSThread.SUSPENDED)
            )
        if "xTasksWaitingTermination" in self._symbols:
            listsToRead.append(
                (self._symbols["xTasksWaitingTermination"], FreeRTOSThread.DELETED)
            )
    ...
    def _get_elf_symbol_size(self, name, addr, calculated_size):
        if self._target.elf is not None:
            ...
            try:
                ...
                LOG.error(
                    "FreeRTOS elf symbol query failed for (%s) with an exception. "
                    + str(e),
                    name,
