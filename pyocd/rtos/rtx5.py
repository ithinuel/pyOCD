    ...
    def __iter__(self) -> Generator[int, Any, None]:
        ...
        while node != 0:
            ...
            try:
                ...
                LOG.warning(
                    "TransferError while reading list elements (list=0x%08x, node=0x%08x), terminating list: %s",
                    self._list,
                    node,
                    exc,
                )
...
class RTXTargetThread(TargetThread):
    ...
    def __init__(
        ...
        try:
            name_ptr = self._target_context.read32(
                self._base + RTXTargetThread.NAME_OFFSET
            )
    ...
    def update_state(self) -> None:
        try:
            ...
            priority = self._target_context.read8(
                self._base + RTXTargetThread.PRIORITY_OFFSET
            )
        except exceptions.TransferError as exc:
            LOG.debug(
                "Transfer error while reading thread %x state: %s", self._base, exc
            )
        else:
            self._state = state
    ...
    def get_stack_pointer(self) -> int:
        ...
        try:
            ...
            LOG.debug(
                ...
                self._base + RTXTargetThread.SP_OFFSET,
            )
            return 0
    ...
    def get_stack_frame(self) -> int:
        ...
        try:
            return (
                self._target_context.read8(
                    self._base + RTXTargetThread.STACKFRAME_OFFSET
                )
                | 0xFFFFFF00
            )
        except exceptions.TransferError:
            LOG.debug(
                "Transfer error while reading thread's stack frame @ 0x%08x",
                self._base + RTXTargetThread.STACKFRAME_OFFSET,
            )
            return 0xFFFFFFFD
...
class RTX5ThreadProvider(ThreadProvider[RTXTargetThread | HandlerModeThread]):
    ...
    def _build_thread_list(self) -> None:
        ...
        thread = self._target_context.read32(
            self._os_rtx_info + RTX5ThreadProvider.CURRENT_OFFSET
        )
        if thread:
        ...
        thread_lists = [
            TargetList(
                ...
                self._readylist,
                RTX5ThreadProvider.THREADNEXT_OFFSET,
            ),
            TargetList(
                self._target_context,
                self._delaylist,
                RTX5ThreadProvider.DELAYNEXT_OFFSET,
            ),
            TargetList(
                self._target_context,
                self._waitlist,
