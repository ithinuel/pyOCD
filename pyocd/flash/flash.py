...
class PageInfo:
    ...
    program_weight: (
        int  # Time it takes to program a page (Not including data transfer time)
    )
...
class Flash:
    ...
    def __init__(self, target, flash_algo):
        ...
        if flash_algo is not None:
            ...
            self.end_flash_algo = (
                flash_algo["load_address"] + len(flash_algo["instructions"]) * 4
            )
    ...
    def init(self, operation, address=None, clock=0, reset=True):
        ...
        assert (self._did_prepare_target) or (
            not self._did_prepare_target and self._active_operation is None
        )
        ...
        if not self._did_prepare_target:
            ...
            self.target.write_memory_block32(
                self.flash_algo["load_address"], self.flash_algo["instructions"]
            )
        ...
        if self._is_api_valid("pc_init"):
            TRACE.debug(
                "algo call init(addr=%d, clock=%d, op=%d)",
                address,
                clock,
                operation.value,
            )
            result = self._call_function_and_wait(
                self.flash_algo["pc_init"],
                r0=address,
                r1=clock,
                r2=operation.value,
                init=True,
    ...
    def compute_crcs(self, sectors):
        ...
        self.target.write_memory_block32(
            self.flash_algo["analyzer_address"], _ANALYZER_CODE
        )
        ...
        self._call_function_and_wait(
            self.flash_algo["analyzer_address"],
            self.begin_data,
            len(data),
    ...
    def erase_sector(self, address):
        ...
        result = self._call_function_and_wait(
            self.flash_algo["pc_erase_sector"],
            address,
        ...
        if result == self.TIMEOUT_ERROR:
            ...
            raise FlashEraseFailure(
                "flash erase sector failure", address=address, result_code=result
            )
    ...
    def program_page(self, address, bytes):
        ...
        TRACE.debug(
            "call program_page(addr=%x, len=%x, data=%x)",
            address,
            len(bytes),
            self.begin_data,
        )
        result = self._call_function_and_wait(
            self.flash_algo["pc_program_page"],
            address,
            len(bytes),
            self.begin_data,
        ...
        if result == self.TIMEOUT_ERROR:
            ...
            raise FlashProgramFailure(
                "flash program page failure", address=address, result_code=result
            )
    ...
    def start_program_page_with_buffer(self, buffer_number, address):
        ...
        # update core register to execute the program_page subroutine
        TRACE.debug(
            "start_program_page_with_buffer(addr=%x, len=%x, data=%x)",
            address,
            page_info.size,
    ...
    def program_phrase(self, address, bytes):
        ...
        if address % min_len:
            raise FlashFailure(
                "flash program phrase failure: unaligned start address", address=address
            )
        ...
        # update core register to execute the program_page subroutine
        TRACE.debug(
            "call program_phrase(addr=%x, len=%x, data=%x)",
            address,
            len(bytes),
            self.begin_data,
        )
        ...
        if result == self.TIMEOUT_ERROR:
            ...
            raise FlashProgramFailure(
                "flash program phrase failure", address=address, result_code=result
            )
    ...
    def _flash_algo_debug_check(self):
        ...
        LOG.debug(
            "Registers after flash algo: [%s]", " ".join("%08x" % r for r in regs)
        )
    ...
    def wait_for_completion(self, timeout=None):
        ...
        if self.end_stack is not None:
            ...
            if canary != self._STACK_CANARY:
                raise exceptions.FlashFailure(
                    f"flash algorithm overflowed stack ({self.begin_stack - self.end_stack} bytes)"
                )

        return self.target.read_core_register("r0")

    def _call_function_and_wait(
        self, pc, r0=None, r1=None, r2=None, r3=None, init=False, timeout=None
    ):
