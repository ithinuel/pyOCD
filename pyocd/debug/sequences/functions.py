...
class DebugSequenceCommonFunctions(DebugSequenceFunctionsDelegate):
        ...
        if pack_device.uses_apid:
            ...
            if dp_num != 0:
                raise DebugSequenceRuntimeError(
                    f"currently only __dp==0 is supported ({dp_num} specified)"
                )
    ...
    def _get_mem_ap(self) -> MEM_AP:
        ...
        try:
            ...
            if not isinstance(ap, MEM_AP):
                raise DebugSequenceRuntimeError(
                    f"AP at address {ap_addr} is not a MEM-AP"
                )
            return ap
        except KeyError:
            pass
    ...
    def sequence(self, name: str) -> None:
        ...
        seq = self.context.delegate.get_sequence_with_name(
            name, pname=self.context.pname
        )
    ...
    def read8(self, addr: int) -> int:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Read8(%#010x) ignored %r because __errorcontrol is set", addr, err
                )
                return 0
            else:
                raise
    ...
    def read16(self, addr: int) -> int:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Read16(%#010x) ignored %r because __errorcontrol is set", addr, err
                )
                return 0
            else:
                raise
    ...
    def read32(self, addr: int) -> int:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Read32(%#010x) ignored %r because __errorcontrol is set", addr, err
                )
                return 0
            else:
                raise
    ...
    def read64(self, addr: int) -> int:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Read64(%#010x) ignored %r because __errorcontrol is set", addr, err
                )
                return 0
            else:
                raise
    ...
    def readap(self, addr: int) -> int:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "ReadAP(%#010x) ignored %r because __errorcontrol is set", addr, err
                )
                return 0
            else:
                raise
    ...
    def readaccessap(self, addr: int) -> int:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "ReadAccessAP(%#010x) ignored %r because __errorcontrol is set",
                    addr,
                    err,
                )
                return 0
    ...
    def readdp(self, addr: int) -> int:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "ReadDP(%#010x) ignored %r because __errorcontrol is set", addr, err
                )
                return 0
            else:
                raise
    ...
    def write8(self, addr: int, val: int) -> None:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Write8(%#010x) ignored %r because __errorcontrol is set", addr, err
                )
            else:
                raise
    ...
    def write16(self, addr: int, val: int) -> None:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Write16(%#010x) ignored %r because __errorcontrol is set",
                    addr,
                    err,
                )
            else:
    ...
    def write32(self, addr: int, val: int) -> None:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Write32(%#010x) ignored %r because __errorcontrol is set",
                    addr,
                    err,
                )
            else:
    ...
    def write64(self, addr: int, val: int) -> None:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "Write64(%#010x) ignored %r because __errorcontrol is set",
                    addr,
                    err,
                )
            else:
    ...
    def writeap(self, addr: int, val: int) -> None:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "WriteAP(%#010x) ignored %r because __errorcontrol is set",
                    addr,
                    err,
                )
            else:
    ...
    def writeaccessap(self, addr: int, val: int) -> None:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "WriteAccessAP(%#010x) ignored %r because __errorcontrol is set",
                    addr,
                    err,
                )
            else:
    ...
    def writedp(self, addr: int, val: int) -> None:
        try:
            ...
            if self._get_ignore_errors():
                LOG.debug(
                    "WriteDP(%#010x) ignored %r because __errorcontrol is set",
                    addr,
                    err,
                )
            else:
    ...
    def message(self, type: int, format: str, *args: int) -> None:
        level = self._MESSAGE_LEVEL_MAP.get(
            type, 2
        )  # default to error for invalid type
