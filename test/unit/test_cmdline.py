...
class TestSplitCommandLine(object):
    ...
    # ;!@#$%^&*()+=[]{}|<>,?
    @pytest.mark.parametrize("sep", list(c for c in ";!@#$%^&*()+=[]{}|<>,?"))
...
class TestConvertVectorCatch(object):
    ...
    @pytest.mark.parametrize(("vc", "msk"), list(VECTOR_CATCH_CHAR_MAP.items()))
    def test_vc_str(self, vc, msk):
        assert convert_vector_catch(vc) == msk

    @pytest.mark.parametrize(
        ("vc", "msk"), [(six.b(x), y) for x, y in VECTOR_CATCH_CHAR_MAP.items()]
    )
