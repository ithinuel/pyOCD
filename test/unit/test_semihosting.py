...
class SemihostRequestBuilder:
    ...
    def do_open(self, filename, mode):
        ...
        self.ctx.write32(
            argsptr + 4, semihost.SemihostAgent.OPEN_MODES.index(mode)
        )  # mode
        self.ctx.write32(
            argsptr + 8, len(filename) - 1
        )  # filename length minus null terminator
    ...
    def do_no_args_call(self, rq):
        self.setup_semihost_request(rq)
...
class TestSemihosting:
    ...
    @pytest.mark.parametrize(
        ("mode", "writeData", "pos", "readLen", "readResult"),
        [
            ...
            (
                "w+",
                "Hello this is an extraaaaaaaa long string",
                0,
                41,
                0,
            ),  # write a long string
            (
                ...
                6 * 2 + 4 + 3,
                0,
            ),  # write string with some UTF-8 encodings
        ],
    )
    ...
    def test_console_write_binary_pattern(self, semihost_builder):
        ...
        binary_pattern = bytes.fromhex(
            "8152666f72706cff08000000000000000000000000000000020000000000000000000000000000000300000000000000000000000000000016000000"
        )
    ...
    def test_console_write0(self, semihost_builder):
        ...
        result = semihost_builder.do_write0(
            b"this is a very looooooooooooooooooooooooooooooooooooooooooooong string with more than 32 characters"
        )
        assert result == 0

        assert (
            console.get_output_data(semihost.STDOUT_FD)
            == b"this is a very looooooooooooooooooooooooooooooooooooooooooooong string with more than 32 characters"
        )

    @pytest.mark.parametrize(
        ("data", "readlen"),
        [(b"12345678", 8), (b"hi", 2), (b"hello", 3), (b"", 4), (b"abcd", 0)],
    )
    ...
    def test_console_readc(self, semihost_builder):
        ...
        result = semihost_builder.do_no_args_call(
            semihost.SemihostingRequests.SYS_READC
        )
        assert chr(result) == "x"

    def test_clock(self, semihost_builder):
        result = semihost_builder.do_no_args_call(
            semihost.SemihostingRequests.SYS_CLOCK
        )
        assert result != -1
        assert result != 0
        logging.info("clock = %d cs", result)

        result2 = semihost_builder.do_no_args_call(
            semihost.SemihostingRequests.SYS_CLOCK
        )
        assert result2 != -1
        assert result2 != 0
    ...
    def test_errno_no_err(self, semihost_builder):
        result = semihost_builder.do_no_args_call(
            semihost.SemihostingRequests.SYS_ERRNO
        )
        assert result == 0
    ...
    def test_feature_bits(self, semihost_builder):
        ...
        assert (
            data[0] & 0x3
        ) == 0x02  # Only check feature bits defined at the time this test was written.
...
class TestSemihostIOHandlerBase:
    ...
    def test_std_open(self, ctx, ramrgn, ioh, filename, mode, expectedFd):
        handler, agent = ioh
        ctx.write_memory_block8(
            ramrgn.start, bytearray(six.ensure_binary(filename) + b"\x00")
        )
        assert handler._std_open(ramrgn.start, len(filename), mode) == (
            expectedFd,
            filename,
        )
