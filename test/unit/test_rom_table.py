...
class MockM4Components:
    ...
    M4_ROM_TABLE = MockCoreSightComponent(
        M4_ROM_TABLE_BASE,
        cidr=0xB105100D,
        pidr=0x4000BB4C4,
        extra={
    ...
    TPIU = MockCoreSightComponent(
        TPIU_BASE, cidr=0xB105900D, pidr=0x4000BB9A1, devtype=0x11, devid=[0xCA1, 0, 0]
    )

    # [5]<e0041000:ETM-M4 class=9 designer=43b part=925 devtype=13 archid=0000 devid=0:0:0>
    ETM_BASE = 0xE0041000
    ETM = MockCoreSightComponent(
        ETM_BASE, cidr=0xB105900D, pidr=0x4000BB925, devtype=0x13
    )
...
class MockCSSOC600Components:
    ...
    C9_ROM_TABLE = MockCoreSightComponent(
        C9_ROM_TABLE_BASE,
        cidr=0xB105900D,
        pidr=0x4000BB7D5,
        devarch=0x47700AF7,
    ...
    SDC600 = MockCoreSightComponent(
        SDC600_BASE, cidr=0xB105900D, pidr=0x4000BB9EF, devarch=0x47700A57
    )

    C9_AHB_AP_BASE = 0x00002000
    C9_AHB_AP = MockCoreSightComponent(
        C9_AHB_AP_BASE, cidr=0xB105900D, pidr=0x4002BB9E3, devarch=0x47700A17
    )
...
class TestMockCoreSight:
    def test_1(self, testcoresight):
        ...
        assert testcoresight.read32(0xE00FF000) == 0xFFF0F003
        assert testcoresight.read_memory_block32(0xE00FFFF0, 4) == [
            0xD,
            0x10,
            0x5,
            0xB1,
...
class TestCoreSightComponentID:
    ...
    def test_scs(self):
        cmp = CoreSightComponentID(
            None, MockCoreSight([MockM4Components.SCS]), MockM4Components.SCS_BASE
        )
    ...
    def test_etm(self):
        cmp = CoreSightComponentID(
            None, MockCoreSight([MockM4Components.ETM]), MockM4Components.ETM_BASE
        )
        cmp.read_id_registers()
    ...
    def test_tpiu(self):
        cmp = CoreSightComponentID(
            None, MockCoreSight([MockM4Components.TPIU]), MockM4Components.TPIU_BASE
        )
        cmp.read_id_registers()
        assert cmp.component_class == 9
        assert cmp.designer == 0x43B
    ...
    def test_c9_rom(self):
        cmp = CoreSightComponentID(
            None,
            MockCoreSight([MockCSSOC600Components.C9_ROM_TABLE]),
...
class TestRomTable:
    ...
    def test_c9_rom(self, c9_top_rom):
        # Read ROM table component ID.
        cmpid = CoreSightComponentID(
            None, c9_top_rom, MockCSSOC600Components.C9_ROM_TABLE_BASE
        )
        cmpid.read_id_registers()
