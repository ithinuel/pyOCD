...
class SequenceDelegateForTesting:
    ...
    def get_sequence_with_name(self, name, pname=None):
        print(
            f"SequenceDelegateForTesting.get_sequence_by_name: name={name} pname={pname}"
        )
        if name == "valid":
            x = mock.Mock()
            print(f"returning {x}")
...
def block_context(context: DebugSequenceExecutionContext):
    ...
    _ctxmgr = context._push(seq, scope)
    return context
...
class TestDebugSequenceScope:
    ...
    def test_copy(self, scope):
        ...
        assert other.is_defined("a") and other.is_defined("b")
...
class TestDebugSequenceParser:
    ...
    def test_assign_minus_negative(self):
        ...
        print("ast=", ast)
        e = (
            ast.children[0].children[0].children[2]
        )  # start.expr_stmt.assign_expr.binary_expr
