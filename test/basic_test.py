...
def basic_test(board_id, file):
    with ConnectHelper.session_with_chosen_probe(
        ...
        addr_flash = (
            rom_region.start + rom_region.length - sector_size * sectors_to_test
        )
        fill = [0x55] * page_size
        for i in range(0, sectors_to_test):
            ...
            if data != [flash.region.erased_byte_value] * sector_size:
                print(
                    "FAILED to erase sector @ 0x%x (%d bytes)" % (address, sector_size)
                )
            else:
                ...
                print(
                    "Verifying programmed page @ 0x%x (%d bytes)" % (address, page_size)
                )
                data = target.read_memory_block8(address, page_size)
                if data != fill:
                    print(
                        "FAILED to program page @ 0x%x (%d bytes)"
                        % (address, page_size)
                    )
        ...
        for i in range(0, sectors_to_test):
            ...
            data = target.read_memory_block8(address, page_size)
            expected = (
                ([flash.region.erased_byte_value] * page_size) if (i == 1) else fill
            )
